...
function StatusTooltip({ entry, pos, onMouseEnter, onMouseLeave }: {
    ...
    return (
      <div
        ...
        style={{
          ...
          padding: '8px 12px', zIndex: 9999, fontSize: 12, color: 'var(--ink-4)',
        }}
      >
        Computing…
      </div>
  ...
  const RED_COLOR  = 'var(--err)';
  const GREEN_COLOR = 'var(--ok)';
    ...
    >
      {shown.map((l, i) => (
        <div key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: 6, marginBottom: i < shown.length - 1 ? 4 : 0 }}>
          ...
          <span style={{ color: 'var(--ink-2)', lineHeight: 1.4 }}>{l.text}</span>
        </div>
      ))}
      {overflow && <div style={{ color: 'var(--ink-4)', marginTop: 4, fontSize: 11 }}>…and {lines.length - MAX} more</div>}
    </div>
  );
}
...
function RevRow({ label, bold, revNote, date, displayName, onClick }: {
  ...
  return (
    <div
      ...
      onClick={onClick}
    >
      <span style={{ fontWeight: bold ? 600 : 500, color: bold ? 'var(--accent-ink)' : 'var(--ink)', minWidth: 96, fontSize: 12 }}>{label}</span>
      {revNote && <span style={{ color: 'var(--ink-2)', fontSize: 12, fontStyle: 'italic' }}>{revNote}</span>}
      <span style={{ color: 'var(--ink-4)', fontSize: 11 }}>—</span>
      <span style={{ color: 'var(--ink-3)', fontSize: 12 }}>{fmtDate(date)}</span>
      <span style={{ color: 'var(--ink-4)', fontSize: 11 }}>—</span>
      <span style={{ color: 'var(--ink-4)', fontSize: 12 }}>{displayName}</span>
    </div>
  );
}
...
export default function QuotesList({
  ...
  return (
    <div className="mcx-card" style={{ maxWidth: 960, margin: '32px auto' }}>
      <div className="mcx-card-head" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
        <span className="mcx-card-title">My Quotes</span>
        <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
          ...
          <input
            ...
            style={{ width: 190 }}
          />
          <button className="mcx-btn is-sm is-quiet" onClick={submitContentSearch} disabled={contentSearching}>
            {contentSearching ? '…' : 'Search'}
          </button>
          {advActive && (
            <button className="mcx-btn is-sm is-quiet" onClick={clearContentSearch}>Clear</button>
          )}
          {canEdit && (
            <button className="mcx-btn is-sm is-primary" onClick={onNew}>+ New Quote</button>
          )}
        </div>
      </div>

      <div className="mcx-card-body" style={{ padding: 0 }}>
            ...
            <tbody>
              {displayList.map(q => {
                ...
                if (statusEntry === 'loading') {
                  ...
                  if (statusEntry.redCount > 0) dotColor = 'var(--err)';
                  else if (statusEntry.warnCount > 0) dotColor = 'var(--warn)';
                  else if (statusEntry.greenCount > 0) dotColor = 'var(--ok)';
                }
                ...
                return (
                  <React.Fragment key={q.id}>
                    <tr style={{ borderBottom: isRevOpen ? 'none' : '1px solid var(--hairline)' }}>
                      ...
                      <td style={{ padding: '8px 12px', color: 'var(--ink-4)', fontFamily: 'monospace', fontSize: 12 }}>
                        {fmtQuoteNum(q.quoteNumber)}
                      </td>
                      <td style={{ padding: '8px 12px', fontWeight: 500 }}>
                        {q.name || '—'}
                        {contentOnly && (
                          <span title="Matched inside the quote contents" style={{ marginLeft: 6, fontSize: 10, color: 'var(--accent-ink)', background: 'var(--accent-tint)', borderRadius: 3, padding: '1px 5px', whiteSpace: 'nowrap' }}>
                            in contents
                          </span>
                        )}
                      </td>
                      <td style={{ padding: '8px 12px', color: 'var(--ink-3)' }}>{q.customer || '—'}</td>
                      <td style={{ padding: '8px 12px', textAlign: 'right', fontFamily: 'monospace', color: 'var(--ink-3)', fontSize: 12, whiteSpace: 'nowrap' }}>
                        {statusEntry && statusEntry !== 'loading' && statusEntry.primaryTotal != null ? fmtC(statusEntry.primaryTotal, 2) : '—'}
                      </td>
                      <td style={{ padding: '8px 12px', textAlign: 'right', color: 'var(--ink-4)', fontSize: 12, whiteSpace: 'nowrap' }}>
                        {fmtDate(q.updatedAt)}
                      </td>
                      <td style={{ padding: '8px 12px', textAlign: 'right', whiteSpace: 'nowrap' }}>
                        <button className="mcx-btn is-sm is-quiet"
                        ...
                        {(role === 'admin' || q.createdBy === userId) && (
                          <button className="mcx-btn is-sm is-quiet" onClick={() => onDelete(q.id)}>✕</button>
                        )}
                      </td>
                    </tr>
                    ...
                    {isRevOpen && (
                      <tr style={{ borderBottom: '1px solid var(--hairline)' }}>
                        <td colSpan={7} style={{ padding: '4px 20px 10px 20px', background: 'var(--surface-2)', borderTop: '1px dashed var(--border)' }}>
                          <RevRow
                            label="Working Draft"
                            bold
                            date={q.updatedAt}
